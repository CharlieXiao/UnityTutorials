...
namespace UnityTutorials.Pseudorandom_Noise
{
    public static class Shapes
    {
        ...
        public struct Job<S> : IJobFor where S : struct, IShape
        {
            ...
            [ReadOnly] public float3x4 positionTRS,normalTRS;
        ...
        {
            float4x2 uv;
            float4 i4 = 4.0f * i + float4(0.0f, 1.0f, 2.0f, 3.0f);
            uv.c1 = floor(invResolution * (i4 + 0.5f));
        ...
        {
            // UVSphere is not uniform distributed
            public Point4 GetPoint4(int i, float resolution, float invResolution)
            {
                float4x2 uv = IndexTo4UV(i, resolution, invResolution);
        ...
        {
            // this one is more uniform distributed than UVSphere
            public Point4 GetPoint4(int i, float resolution, float invResolution)
            {
                float4x2 uv = IndexTo4UV(i, resolution, invResolution);
                ...
                // turn a octahedron to a sphere by normalize the vector length to 0.5
